...
function practice_encodingLoopBegin(practice_encodingLoopScheduler, snapshot) {
  return async function() {
    ...
    for (const thisPractice_encoding of practice_encoding) {
...
function practice_recallLoopBegin(practice_recallLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisPractice_recall of practice_recall) {
...
function word_loopLoopBegin(word_loopLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisWord_loop of word_loop) {
...
function recall_loopLoopBegin(recall_loopLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisRecall_loop of recall_loop) {
...
function blank_1sRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of blank_1sComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function start_recallRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of start_recallComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function exitRoutineRoutineBegin(snapshot) {
  return async function () {
    ...
    fetch('https://pipe.jspsych.org/api/data', {
        ...
        body: JSON.stringify({
            experimentID: 'QrKLJNngYKZM', // ⭑ UPDATE WITH YOUR DATAPIPE EXPERIMENT ID ⭑
            filename: filename,
            data: data,
        }),
    ...
    exitRoutineComponents.push(text_8);
